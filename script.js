...
if(homeCategoryNav&&homeNomadicTrigger&&homeNomadicSubmenu){
  ...
  const parentLinks=[...homeCategoryNav.querySelectorAll(':scope > a')];
  ...
  parentLinks.forEach((link)=>link.addEventListener('click',(event)=>{event.preventDefault();if(previewLink===link&&introPanel?.classList.contains('is-open')){closeCategoryIntro();return;}setHomeNomadicOpen(false);parentLinks.forEach((item)=>item.classList.toggle('is-active',item===link));activeHomeCategory=new URL(link.href).searchParams.get('category')||'nitunxiang';renderHomeArticle(articleForCategory(activeHomeCategory,link.dataset.homeArticle));showCategoryIntro(link);}));
  ...
  const initialHomeLink=parentLinks.find((link)=>link.classList.contains('is-active'));activeHomeCategory=new URL(initialHomeLink?.href||location.href).searchParams.get('category')||'nitunxiang';renderHomeArticle(articleForCategory(activeHomeCategory,initialHomeLink?.dataset.homeArticle));
  window.NTX_ARTICLES?.ready?.then(()=>{const currentLink=parentLinks.find((link)=>link.classList.contains('is-active'))||initialHomeLink;activeHomeCategory=new URL(currentLink?.href||location.href).searchParams.get('category')||'nomadic';parentLinks.forEach((link)=>link.classList.toggle('is-active',link===currentLink));renderHomeArticle(articleForCategory(activeHomeCategory,currentLink?.dataset.homeArticle));});
}
...
loadInitialState().finally(async() => { window.syncHomeArticle?.();syncManifestoEyebrowTop();await Promise.all([revealHomeWhenReady(),document.fonts?.ready||Promise.resolve()]);document.querySelectorAll('.hero .reveal').forEach((node)=>node.classList.add('is-visible'));if(!location.hash||location.hash==='#top')scrollTo(0,0);requestAnimationFrame(()=>document.documentElement.classList.remove('state-loading')); });

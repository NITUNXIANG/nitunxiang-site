...
loadInitialState().finally(async() => { window.syncHomeArticle?.();syncManifestoEyebrowTop();await Promise.all([revealHomeWhenReady(),document.fonts?.ready||Promise.resolve()]);document.querySelectorAll('.hero .reveal').forEach((node)=>node.classList.add('is-visible'));requestAnimationFrame(()=>document.documentElement.classList.remove('state-loading')); });

(function(){
  ...
  const syncGuideState=()=>{const open=Boolean(expandedCategory);document.body.classList.toggle('detail-guide-open',open);document.body.classList.toggle('detail-guide-visual-open',open&&guideMode==='visual')};
  document.querySelectorAll('[data-detail-guide-mode]').forEach((button)=>button.addEventListener('click',()=>{guideMode=button.dataset.detailGuideMode;categoryResults.dataset.guideMode=guideMode;document.querySelectorAll('[data-detail-guide-mode]').forEach((item)=>{const active=item===button;item.classList.toggle('is-active',active);item.setAttribute('aria-pressed',String(active))});syncGuideState()}));

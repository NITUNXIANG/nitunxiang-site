(function(){
  ...
  const renderCategoryResults=(category)=>{if(!categoryResults)return;const entries=Object.entries(window.NTX_ARTICLES?.all?.()||{}).filter(([,article])=>(article.category||'nitunxiang')===category);categoryResults.innerHTML=entries.map(([slug,article],index)=>{const href=`article-detail.html?article=${encodeURIComponent(slug)}`,number=String(index+1).padStart(2,'0');return `<a class="detail-category-visual" href="${href}"><time>${article.date||number}</time><figure><img src="${article.images?.[0]||''}" alt=""></figure><div class="detail-category-visual-copy"><small>${article.type?.[0]||''}</small><h3>${article.title?.[0]||article.title?.[1]||''}</h3><p>${article.dek?.[0]||article.dek?.[1]||''}</p></div><b>↗</b></a><a class="detail-category-result" href="${href}"><span>${article.title?.[1]||article.title?.[0]||''}</span><i>${article.title?.[0]||''}</i><b>${number}</b></a>`}).join('');categoryResults.hidden=!entries.length};
